...
public class Visit_Analyst {
    ...
    public Visit_Analyst(String id, String client_name, String structure_name,
                         String service_name, String data, String employee_name, String summa, String points, String status) {
        this.id = id;
    ...
    public static Pane getPane(Pane pane, ScrollPane scrollPane) throws SQLException, FileNotFoundException, ClassNotFoundException {
        ...
        Text st = new Text("Услуга оказана");
        st.setLayoutX(780);
        st.setLayoutY(60);
        st.setFont(Font.font("Verdana",13));

        Visit_Analyst[] mas = DataBase.getAllVisit_Analyst();
        int u = 80;
        for(int i=0; i<mas.length;i++, u+=70){
            ...
            status_text.setLayoutX(750);
            status_text.setLayoutY(0 + u);
            status_text.setMaxHeight(40);
            status_text.setMaxWidth(80);
            status_text.setMinHeight(40);
            status_text.setMinWidth(80);

            pane.getChildren().addAll(num_text,cl_text,emp_text,ser_text,price_text,str_text,point_text, data_text, status_text);
        }

        res.getChildren().addAll(num, cl, em, ser, price, str, point, date, st);
        return res;
    }
